...
export default function OrderDetailsPage() {
  ...
  const { roomRules } = useRoomTypeRules();
  ...
  // ⭐ FIX 2: Check if order exists too
  const isLoading = orderLoading || supplementsLoading || transportLoading || !order;

  // ⭐ FIX 3: Wait for transport types to load + add delay
  useEffect(() => {
    if (!order || orderLoading || transportLoading) return;
  ...
  }, [order, orderLoading, transportLoading, form]);
