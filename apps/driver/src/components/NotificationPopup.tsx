...
export function NotificationPopup() {
  ...
  useEffect(() => {
    ...
    if (permission === 'default') {
      const dismissed = localStorage.getItem('driver-notification-popup-dismissed');
      if (!dismissed) {
  ...
  const handleEnable = async () => {
    ...
    if (result.success) {
      setShow(false);
      localStorage.setItem('driver-notification-popup-dismissed', 'true');
  ...
  if (!mounted || !isAuthenticated || permission !== 'default' || !show) return null;
          ...
          <div className="p-6">
            <div className="flex items-start gap-4">
              ...
              <div className="flex-1 min-w-0">
                <h3 className="text-lg font-semibold text-gray-900 mb-1">Enable Tour Notifications</h3>
                <p className="text-sm text-gray-600 mb-4">
                  Get instant alerts when a tour is assigned to you — even when the app is closed.
                </p>
                {errorMsg && (
                ...
                <div className="flex items-center gap-3">
                  <Button onClick={handleEnable} disabled={loading} className="bg-blue-600 hover:bg-blue-700">
                    {loading ? 'Requesting...' : 'Enable Notifications'}
                  </Button>
                  <Button onClick={handleDismiss} variant="ghost" className="text-gray-600">
                    Maybe Later
                  </Button>
                </div>
              </div>
